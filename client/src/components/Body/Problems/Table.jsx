...
import load from '../../../videos/unboxing-unscreen.mp4'
...
export default function EnhancedTable() {
  ...
  const handleClick = (row) => {
    ...
    // setProblemData(row)
    // setOpen(true)
    navigate(`/compiler/${row.title}/${row._id}`)
  };
  ...
  const deleteProblem = async (row) =>{
    ...
    try {
      const res = await fetch(`/deleteProblem/${row._id}`,{
        method:'POST',
        headers : {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right" className='Action'>
                        {
                          rootUser._id === row.owner._id ? <div className='action_button'>
